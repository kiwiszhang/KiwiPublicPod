...
extension String {
    ...
    ///
    /// - Returns: 是否是正确密码
    public func isValidatePSD() -> Bool {
    ...
    /// - Returns: 处理后手机号
    public func numberSuitScanf() -> String {
    ...
    ///   - str: 原有字符
    ///   - length: 长度
    public func centerSuidScanfStr(_ length: NSInteger) -> String? {
    ...
    /// - Parameter time: 时间
    /// - Returns: 结果字符串
    public static func timeStr(time: NSInteger) -> String {
        if time <= 0 {
            return "00:00"
        }
    ...
    /**
     清除乱七八糟的字符
     */
    public func cleaningUString() -> String {
    ...
    /**
     判断是否不是自己的聊天
        消息发送人是以 ～ 和 + 号开头的
     */
    public func isOtherSender() -> Bool {
...
/// 将字符串时间转为时间戳（秒级）
    ...
    public func toTimestamp(format: String = "yyyy-MM-dd HH:mm:ss",
    ...
    /// 转换为毫秒级时间戳
    public func toTimestampMillis(format: String = "yyyy-MM-dd HH:mm:ss",
                           timeZone: TimeZone = TimeZone.current) -> Int64? {
        let formatter = DateFormatter()
        formatter.dateFormat = format
        formatter.locale = Locale(identifier: "en_US_POSIX")
    ...
    /// 字符串时间戳（秒）→ 日期字符串
    public func timestampToDateString(format: String = "MMM dd, yyyy",
...
extension String {
    public func decodeJSON<T: Decodable>(to type: T.Type) -> T? {
